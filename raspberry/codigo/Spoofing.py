...
logging.basicConfig(filename=f"spoofing-{datetime.datetime.now()}.log", level=logging.INFO, format='%(message)s')
...
try:
    while True:
        for msg in bus: 
            ...
            # Também copia o timestamp
            spoofed_timestamp = msg.timestamp
            message = can.Message(arbitration_id=spoofed_id, timestamp=spoofed_timestamp, data = [random.randint(0, 255) for _ in range(8)], is_extended_id=False)

...
logging.basicConfig(filename=f"reply-{datetime.datetime.now()}.log", level=logging.INFO, format='%(message)s')
...
try:
    ...
    while True:   
        spoofed_id = copied_msg.arbitration_id
        current_time = time.time()
        elapsed_time = current_time - message_time
        message = can.Message(arbitration_id=spoofed_id, timestamp=spoofed_time+elapsed_time, data = copied_msg.data, is_extended_id=False)
